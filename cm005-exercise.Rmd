...
Note: think of `%>%` (ctrl + shift + m) as the word "then"!

1. Combine `select()` Task 1 with `arrange()` Task 3.
select(gapminder, year, lifeExp, country)
arrage(gapminder, year, lifeExp)
```{r}
gapminder %>% 
...
```{r}
gapminder %>% 
    filter(pop > 100000000) # filter requires logical statement, returns rows corresponding TRUE to statement
```
...
gapminder %>% 
    filter(
      (country == "Canada" | country == "Algeria") & 
        year >= 1960 & year < 1970)
