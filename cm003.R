...
typeof(my_commute) # type double -> number (can have decimals)

## Let's use the `paste` function to print out the preamble followed by
##  the commute time:
paste(preamble, my_commute) # "My commute time, in minutes: 70"
?paste # for info or help doc on this paste function
...
commute <- as.numeric(c(my_commute, 25, 15, 5, 35, 15, 50))

## Use the `length` function to store the number of observations in `n`.
n <- length(commute)
(n <- length(commute)) #  prints out n
